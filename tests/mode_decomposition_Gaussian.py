...
if input_type == "random":
    ...
    input_beam = Input(domain, wvl0, n_core, n_clad, type="gaussian", noise=True, beam_radius=beam_radius, amp=amp, radius=radius, device=device)
else:
...
fields = np.zeros((100, combined_field.shape[0], combined_field.shape[1]), dtype=complex)
for i in range(100):
    propagation_constants = 2 * np.pi * effective_indices / wvl0
    propagation_length = i * 0.01
    ...
    fields[i] = combined_field_prop

np.save('./data/linear_fields.npy', fields)
...
for i in range(num_modes):
    mode_coeff_abs = np.abs(mode_coefficients)
    l, m = n_to_lm(i+1)
    print(f'l : {l}, m : {m}, mode_coeff_abs : {mode_coeff_abs[i]}')
