...
for total_power in powers:
    ...
    input_beam = Input(domain, wvl0, n_core, n_clad, type="mode_mixing", power=total_power, 
                        radius=fiber_radius, num_mode=num_modes, cx=cx, cy=cy, coefficients=coefficients, device=device)
    ...
    make_3d_animation(fields, indices=fiber_index, radius=fiber_radius/unit, filename=animation_filename, extent=extent, interpolation="bilinear")
