...
parser.add_argument('--input_type', type=str, default='gaussian', choices=['gaussian', 'mode', 'fundamental'], help='Input type')
parser.add_argument('--num_modes', type=int, default=10, help='Number of input modes')
parser.add_argument('--total_power', type=float, default=10, help='Total power (W)')
parser.add_argument('--beam_radius', type=float, default=10e-6, help='Beam radius (m)')
...
dA = dx * dy
norm = np.sqrt(np.sum(np.abs(input_field)**2) * dA)
# input_field_scaled = input_field * np.sqrt(total_power) / norm
# norm = np.sqrt(np.sum(np.abs(input_field_scaled - input_field)**2) * dA)
# print(f'The input field is scaled by {norm:.2f} to match the total power of {total_power * 1e-3:.2f} kW')
...
print(f'The numerical nonlinear phase is {nl_phase} rad')
