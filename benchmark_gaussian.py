...
from src.util import plot_index_profile, plot_beam_intensity, plot_beam_intensity_and_phase, make_3d_animation
...
fiber = Fiber(domain, n_core, n_clad, total_z, dz, n2=n2, radius=fiber_radius, precision=precision, device=device)
...
input_beam = Input(domain, wvl0, n_core, n_clad, 
                    type="gaussian", cx=cx, cy=cy, pixels=pixels, 
                    power=total_power, noise=noise, precision=precision,
                    beam_radius=beam_radius, radius=fiber_radius, device=device)
...
fields, energies  = run(domain, input_beam, fiber, wvl0, dz=dz, n_sample=num_sample, precision=precision, disorder=disorder, ds_factor=ds_factor)
...
# np.save(f'energies_{fiber_radius}_{beam_radius}_{position}_{int(total_power)}.npy', energies)
...
if disorder:
    animation_filename = f'./results/{input_type}_{beam_radius}_{position}_{num_pixels}x{num_pixels}_disorder_{int(total_power)}_noiseX'
else:
    animation_filename = f'./results/{input_type}_{beam_radius}_{position}_{num_pixels}x{num_pixels}_{int(total_power)}_noiseX'
